...
export class Game {
	...
	constructor(options: GameOptions) {
		...
		this.grid = utils.create_empty_grid(
			curr_options.rows,
			curr_options.cols,
			() => ({ balls: [], player: null }),
		);
	...
	tap(row: number, col: number): null | { ball: m.Ball; blast: boolean } {
		...
		const blast = this.get_max_allowed_balls(row, col) < box.balls.length;
		return { ball, blast };
	}
	...
	should_be_blasted(row_idx: number, col_idx: number) {}
	...
	blast_one(): BlastResult {
		...
		let old_grid = utils.create_empty_grid(rows, cols, () => ({
			balls: [],
			player: null,
		}));
			...
			for (const j of Array(cols).keys()) {
				...
				for (const name of DIRECTIONS) {
					...
					this.grid[i][j].player =
						this.grid[i][j].balls.length == 0 ? null : this.curr_player;
